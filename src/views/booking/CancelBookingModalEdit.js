...
const CancelBookingModalEdit = ({ selectedBooking, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (selectedBooking) {
      let feeDetails = selectedBooking?.feeDetails?.feeDetails?.map((fee) => {
        return {
          type: fee.type,
          title: fee.title,
          value: fee.inputValue,
          amount: fee.calculatedAmount,
          id: fee.feeId,
        };
      });

      const totalReturnAmount = feeDetails.reduce(
        (sum, f) => sum + Number(f.amount),
        0
      );

      setReturnAmount(selectedBooking?.totalPayable - totalReturnAmount);
      setCancelRequest({
        customerPayableId: selectedBooking?.id,
  ...
  const onChangeFeesRequest = (e, ind) => {
    ...
    let valueInput = updatedRequest.fees[ind]["value"];

    let totalAmountSpecific = 0;

    if (type == "FIXED") {
      totalAmountSpecific = valueInput;
    } else if (type == "PERCENTILE") {
      totalAmountSpecific = (selectedBooking?.totalPayable / 100) * valueInput;
    }

    updatedRequest.fees[ind]["amount"] = totalAmountSpecific;
    ...
    setReturnAmount(selectedBooking?.totalPayable - total);
  };
  ...
  const onSubmit = async () => {
    try {
      ...
      await httpService.post(
        `/customerpayable/editCustomerPayable`,
        cancelRequest
      );

      await notifySuccess("Successfully updated!", 4000);
      setLoading(false);
    } catch (err) {
              ...
              <h6 className=" text-blueGray-600 text-sm mt-3 mb-6 font-bold uppercase flex justify-between">
                <div className="flex justify-between">
                  ...
                  <div className="pt-2 pr-2  px-2">
                    Total Refund :{" "}
                    <span style={{ fontSize: "23px" }}>
                      {parseFloat(returnAmount).toLocaleString()}
                    </span>
                  </div>
                  <div className="pt-2 pr-2  px-2">
                    Total Returned :{" "}
                    <span style={{ fontSize: "23px" }}>
                      {parseFloat(selectedBooking?.totalPaid).toLocaleString()}
                    </span>
                  </div>
                </div>
                <button
...
export default CancelBookingModalEdit;
